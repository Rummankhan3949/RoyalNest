...
/// Admin AI Assistant Screen - Admin-focused AI copilot
...
class _AdminAIAssistantScreenState extends State<AdminAIAssistantScreen> {
  ...
  final List<String> _suggestedPrompts = const [
    ...
    'Which users have overdue installments?',
    'Summarize lost & found claims this week.',
    'What are today\'s admin priorities?',
  ];
  ...
  Future<void> _ensureAdminAccess() async {
    final snapshot = await _sessionCacheService.readSession();
    if (!mounted) return;

    if (!snapshot.isLoggedIn || !snapshot.isAdmin) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Admin access only. Please sign in as admin.'),
        ),
      );
  ...
  Future<void> _handleSendMessage() async {
    ...
    try {
      final reply = await _assistantService.reply(
        userMessage: message,
        currentScreen: 'Admin AI Assistant',
        userRole: 'admin',
      );
      if (!mounted) return;
      ...
      _addMessage(
        text:
            ...
            '3) Run with --dart-define=GEMINI_API_KEY=YOUR_KEY if needed.\n\n'
            'Error: $e',
        isUser: false,
      );
    }
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              const Text(
                'Paste your Gemini API key. It is saved only on this device.',
              ),
              ...
              onPressed: () async {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      key.isEmpty
                          ? 'API key cleared. Admin AI will use fallback.'
  ...
  Widget build(BuildContext context) {
    return Scaffold(
            ...
            onPressed: () {
              showDialog(
                ...
                builder: (context) => AlertDialog(
                  ...
                  title: const Text('About Admin AI'),
                  content: const Text(
                    'Admin AI helps with workflows across users, properties, '
                    'payments, approvals, moderation, and analytics.',
                  ),
                  actions: [
                    TextButton(
      ...
      body: Column(
        children: [
          if (_isCheckingAiKey || _isCheckingAccess)
          ...
          Expanded(
            child: AnimatedSwitcher(
              ...
              child: _messages.isEmpty
                  ? _buildEmptyState()
                  : ListView.builder(
                      key: const ValueKey('admin-messages'),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: const LinearGradient(
                ...
                colors: [Color(0xFF0A2E73), Color(0xFF0040CC)],
              ),
            ...
            child: const Column(
              ...
              children: [
                ...
                SizedBox(height: 8),
                Text(
                  'Ask about users, properties, payments, approvals, '
                  'moderation, or dashboard insights.',
